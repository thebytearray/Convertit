...
class MetadataRepositoryImpl @Inject constructor(
            ...
            try {
                ...
                parcelFileDescriptor.use { fd ->
                    ...
                    Metadata.fromPropertyMap(
                        taglibMetadata.propertyMap, taglibMetadata.pictures.toList()
                    )
                }
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    override suspend fun saveMetadata(audioUri: Uri, metadata: Metadata): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val parcelFileDescriptor = context.contentResolver.openFileDescriptor(audioUri, "rw")
    ...
    override suspend fun saveCoverArt(audioUri: Uri, bitmap: Bitmap?): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val parcelFileDescriptor = context.contentResolver.openFileDescriptor(audioUri, "rw")
                    ?: return@withContext false
